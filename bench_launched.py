...
if __name__ == "__main__":
    ...
    with open("deleteme.csv", "w") as f:
        ...
        f.write("Filename,Codegen(ms),Compile(ms),SABLE(us)\n")
        for file_path in matrices:
            ...
            try:
                ...
                if not success:
                    f.write(f"{fname},ERROR2,ERROR2,ERROR2\n")
                    f.flush()
                    print(f"Done {fname}")
                    continue
                ...
                f.write(f"{fname},{codegen_time}ms,{compile_time}ms,{statistics.median(execution_time_unroll)}us\n")
                f.flush()
                print(f"Done {fname}")
            except Exception as e:
                f.write(f"{fname},ERROR3,ERROR3,ERROR3\n")
                f.flush()
