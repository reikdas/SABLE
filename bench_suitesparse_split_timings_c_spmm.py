...
from src.codegen import gen_single_threaded_spmm_naive, gen_single_threaded_spmm_spreg
...
GENERATED_SPMM_NAIVE_DIR = FILEPATH / "Generated_SpMM_C_naive"
GENERATED_SPMM_SPREG_DIR = FILEPATH / "Generated_SpMM_C_spreg"
...
def process_and_benchmark_matrix_naive(
    ...
    """
    Run SpMM benchmarks using naive implementation with pre-converted VBRC data.
    ...
    """
    codegen_dir_split = str(GENERATED_SPMM_NAIVE_DIR / "split")
    codegen_dir_sparse = str(GENERATED_SPMM_NAIVE_DIR / "sparse")
...
def process_and_benchmark_matrix_spreg(
    matrix_name: str,
    split_vbrc_data: Dict[str, Any],
    sparse_vbrc_data: Dict[str, Any],
    matrix_rows: int,
    ...
    """
    Run SpMM benchmarks using sparse-register-tiling with pre-converted VBRC data.
    ...
    Returns:
        Dictionary with benchmark results
    """
    codegen_dir_split = str(GENERATED_SPMM_SPREG_DIR / "split")
    codegen_dir_sparse = str(GENERATED_SPMM_SPREG_DIR / "sparse")
    ...
    # Generate C code for split version
    print(f"  [spreg] Generating C code (split)...")
    ...
    # Evaluate fully sparse version (compile + run)
    print(f"  [spreg] Evaluating fully sparse version...")
...
def main():
    ...
    parser.add_argument("--naive-only", action="store_true",
                        help="Only run the naive SpMM benchmark (skip sparse-register-tiling)")
    parser.add_argument("--spreg-only", action="store_true",
                        help="Only run the sparse-register-tiling SpMM benchmark (skip naive)")
    
    args = parser.parse_args()
    
    # Validate mutually exclusive options
    if args.naive_only and args.spreg_only:
        print("Error: --naive-only and --spreg-only are mutually exclusive")
        return 1
    
    # Determine which benchmarks to run
    run_naive = not args.spreg_only
    run_spreg = not args.naive_only
    ...
    benchmark_types = []
    if run_naive:
        benchmark_types.append("naive")
    if run_spreg:
        benchmark_types.append("spreg")
    ...
    for matrix_name in matrices:
        ...
        try:
            ...
            # Run naive version (if enabled)
            if run_naive:
                print(f"\n  === Running naive SpMM benchmark ===")
                ...
                if naive_result:
                    ...
                    naive_output = output_dir / "sable_spmm_naive.json"
                    with open(naive_output, 'w') as f:
                        json.dump(naive_results, f, indent=2)
                    print(f"  [naive] Results written to {naive_output}")
            
            # Run sparse-register-tiling version (if enabled)
            if run_spreg:
                print(f"\n  === Running sparse-register-tiling SpMM benchmark ===")
                spreg_result = process_and_benchmark_matrix_spreg(
                    matrix_name, split_vbrc_data, sparse_vbrc_data,
                    matrix_rows, matrix_cols, matrix_nnz,
                    args.bench
                )
                if spreg_result:
                    # Update or append result
                    existing_idx = next((i for i, r in enumerate(spreg_results) if r["matrix_name"] == matrix_name), None)
                    if existing_idx is not None:
                        spreg_results[existing_idx] = spreg_result
                        print(f"  [spreg] Updated existing result for {matrix_name}")
                    else:
                        spreg_results.append(spreg_result)
                        print(f"  [spreg] Added new result for {matrix_name}")
                    
                    # Write intermediate results
                    spreg_output = output_dir / "sable_spmm_spreg.json"
                    with open(spreg_output, 'w') as f:
                        json.dump(spreg_results, f, indent=2)
                    print(f"  [spreg] Results written to {spreg_output}")
    ...
    if run_naive and naive_results:
        print(f"\nNaive SpMM Results ({len(naive_results)} matrices):")
    ...
    if run_spreg and spreg_results:
        print(f"\nSparse-Register-Tiling SpMM Results ({len(spreg_results)} matrices):")
        for result in spreg_results:
            num_dense_blocks = len(result['individual_dense_block_timings'])
            speedup_dispatch = result['timing'].get('speedup', 0)
            print(f"  {result['matrix_name']}: {num_dense_blocks} dense blocks, "
                  f"sparse: {result['timing']['sparse_time_ns']:.0f}ns, "
