...
from src.autopartition import my_convert_dense_to_vbr, cut_indices1
...
def bench_spmv():
    ...
    vbr_dir = pathlib.Path(os.path.join(BASE_PATH, "Suitesparse_vbr_thresh0.1_cut1_1thread"))
    codegen_dir = os.path.join(BASE_PATH, "Generated_SpMV_suitesparse_thresh0.1_cut1")
    ...
    for thread in threads:
        file_handles[thread] = open(os.path.join(BASE_PATH, "results", f"benchmarks_spmv_suitesparse_thresh0.1_cut1_{thread}thread.csv"), "w")
        ...
        if file_path.is_file() and file_path.suffix == ".mtx" and check_file_matches_parent_dir(file_path):
            ...
            with tqdm(total=(len(threads)*7) + 2, desc="Steps", leave=False) as step_bar:
                try:
                    ...
                    my_convert_dense_to_vbr((str(file_path), str(dest_path)), 0.1, cut_indices1)
                    step_bar.update(1)
                    # Check partitioning
