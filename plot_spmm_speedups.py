...
def parse_filename(filename):
    """Extract dense and sparse from filename like sable_spmm_{dense}_{sparse}.json
    Excludes files with pattern sable_spmm_{dense}_{sparse}_{matrix}.json"""
    ...
    # Exclude files with 3 or more parts where the third part matches a matrix name
    if len(parts) >= 3:
        ...
        dense, sparse = parts[0], '_'.join(parts[1:])
        return dense, sparse
...
def load_all_data(results_dir='results'):
    """Load all spmm result files and organize by (dense, sparse) combination."""
    ...
    for json_file in json_files:
        dense, sparse = parse_filename(json_file)
        if dense is None or sparse is None:
            print(f"Warning: Could not parse filename {json_file}, skipping")
            continue
        if dense == 'naive':
            continue
        ...
        for entry in data:
            ...
            all_data[(dense, sparse)][matrix_name] = entry
...
def plot_speedup_vs_theoretical(all_data, matrix_nnz, output_prefix):
    ...
    for dense, sparse in combinations:
        ...
        ax.set_title(f'SpMM Speedups: dense={dense}, sparse={sparse}\n'
...
def plot_best_times_comparison(all_data, matrix_nnz, output_prefix):
    ...
    with PdfPages(combined_pdf_path) as pdf:
        ...
        ax.set_title('SpMM Speedup: Best SABLE Configuration vs Best Fully Sparse\n'
...
def main():
    parser = argparse.ArgumentParser(description="Plot SpMM speedups from results JSON files.")
    parser.add_argument(
        "--results-dir",
        default="results",
        help="Directory containing sable_spmm_*.json results (default: results)",
    )
    parser.add_argument(
        "--dense",
        default=None,
        help="Filter to a single dense dispatch (e.g., mkl). If omitted, plot all.",
    )
    args = parser.parse_args()

    results_dir = args.results_dir

    print("Loading SpMM result data...")
    all_data, matrix_nnz = load_all_data(results_dir)

    if args.dense is not None:
        all_data = {k: v for k, v in all_data.items() if k[0] == args.dense}
    ...
    speedup_prefix = os.path.join(results_dir, 'spmm_speedup_vs_theoretical')
    ...
    best_times_prefix = os.path.join(results_dir, 'spmm_best_times_comparison')
