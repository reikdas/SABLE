...
def plot_relmatrix(matrix_name):
    ...
    for name in names:
        with open(f"{BASE_PATH}/SABLE/results/benchmarks_spmm_1_rel.csv", "r") as f:
            for line in f:
                parts = line.split(",")
        ...
        with open(f"{BASE_PATH}/SABLE/results/benchmarks_spmm_1_rel_bad.csv", "r") as f:
            for line in f:
                parts = line.split(",")
                # if parts[0] == name+"_uniform":
        ...
        with open(f"{BASE_PATH}/SABLE/sparse-register-tiling/results/bench_executor_1thrds_rel.csv", "r") as f:
            for line in f:
                parts = line.split(",")
                # if parts[0] == name+"_uniform":
    ...
    plt.plot(zerosl, speedup_nonuniform_bad, label="SABLE vs Sparse Register Tiling")
    plt.plot(zerosl, speedup_nonuniform, label="SABLE vs Sparse Register Tiling (Unrolled loops for sparse blocks)")
    plt.xlabel("Percentage of zeros per dense block", fontsize=12)
    plt.ylabel("Speedup", fontsize=12)
    plt.subplots_adjust(bottom=0.23)
    plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.15))
    plt.axhline(y=1, color='b', linestyle='--')
    plt.savefig("relsparse_spmm.pdf")
