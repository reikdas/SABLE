...
def plot_relmatrix(matrix_name):
    ...
    for name in names:
        with open(f"{BASE_PATH}/SABLE/results/benchmarks_spmm_1.csv", "r") as f:
            for line in f:
                parts = line.split(",")
        ...
        with open(f"{BASE_PATH}/SABLE/sparse-register-tiling/results/bench_executor_1thrds.csv", "r") as f:
            for line in f:
                parts = line.split(",")
                # if parts[0] == name+"_uniform":
    ...
    plt.savefig("relspeedup_spmm.pdf")
