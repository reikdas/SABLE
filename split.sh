#!/usr/bin/env bash
# Takes $FILE and replaces K spmv_kernel-calls in it with $N calls to spmv_kerneli,
# spmv_kerneli are functions defined in separate files spmv_kerneli.c (10<=i<N+10).
...
echo "0. Cleanup old spmv_kerneli's and restore original C file"
rm -f spmv_kernel*.c spmv_kernel*.h
...
echo '1. Extract the original spmv_kernel-calls in `body.ins` and put spmv_kernel$i-calls instead'
taskset -a -c $AFFINITY runhaskell "$SCRIPT_DIR/replace_spmv_kernel.hs" $N $FILE split-and-binaries/$SAVE_DIR
echo "2. Split 'body.ins' into N"
split -n r/$N $SCRIPT_DIR/split-and-binaries/$SAVE_DIR/body.ins $SCRIPT_DIR/split-and-binaries/$SAVE_DIR/spmv_kernel --numeric-suffixes=10 --additional-suffix=".c"
echo "3. Patch the spmv_kernel\$i.c: add funciton header and footer"
for ((i=10; i<=$N1; i++)); do
#    sed -i "1i void foo$i(float *x, float *y, float *val) {" "foo$i.c"
    sed -i "1i #include \"spmv_kernel.h\"\nvoid spmv_kernel$i(float *x, float *y, float *val) {" "$SCRIPT_DIR/split-and-binaries/$SAVE_DIR/spmv_kernel$i.c"
    echo "}" >> "$SCRIPT_DIR/split-and-binaries/$SAVE_DIR/spmv_kernel$i.c"
done
echo "4. Create spmv_kernel.h with the SpMV kernel (function spmv_kernel)"
echo "
inline
...
" > $SCRIPT_DIR/split-and-binaries/$SAVE_DIR/spmv_kernel.h
echo "5. Compiling and linking the result"
for f in $SCRIPT_DIR/split-and-binaries/$SAVE_DIR/spmv_kernel*.c; do
...
taskset -a -c $AFFINITY gcc -O3 -mavx -mprefer-vector-width=512 -funroll-all-loops -o $SCRIPT_DIR/split-and-binaries/$SAVE_DIR/$SAVE_DIR ${FILE%.c}.o $SCRIPT_DIR/split-and-binaries/$SAVE_DIR/spmv_kernel*.o