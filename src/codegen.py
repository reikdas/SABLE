...
int lcm(int a, int b) {
    ...
    code.append("\t__m512 bv, cv;\n")
    code.append("\t __m512 sv[10];\n")
    count = 0
    for a in range(len(rpntr)-1):
        if bpntrb[a] == -1:
            continue
        ...
        for b in range(len(cpntr)-1):
            if b in valid_cols:
                code.append(f"\tfor (int i={rpntr[a]}; i<{rpntr[a+1]}; i++) {{\n")
                code.append(f"\t\tfor (int k={cpntr[b]}; k<{cpntr[b+1]}; k+=10) {{\n")
                code.append("\t\t\tfor (int n=0; n<10; n++) {\n")
                ...
                code.append("\t\t\t\tfor (int n=0; n<10; n++) {\n")
                code.append("\t\t\t\t\tbv = _mm512_loadu_ps(&x[(k+n)*512 + 16*j]);\n")
                code.append("\t\t\t\t\tcv = _mm512_fmadd_ps(sv[n], bv, cv);\n")
                code.append("\t\t\t\t}\n")
