...
def _detect_mkl_config() -> tuple[list[str], bool]:
    ...
    # 2. nix-shell: compiler wrapper already has MKL paths
    nix_cflags = os.environ.get("NIX_CFLAGS_COMPILE", "")
    if any("mkl" in token.lower() for token in nix_cflags.split()):
        return ["-lmkl_rt"], True
...
def build_compile_command(
    ...
    The *backend* selects sparse-kernel-specific sources and flags from
    :data:`BACKEND_EXTRA_SOURCES` / :data:`BACKEND_FLAGS`.  When *dense_kernel*
    is :attr:`DenseKernel.BLAS`, MKL flags are added automatically (``cblas_dgemv``
    lives in MKL) unless the backend is already MKL.
    """
