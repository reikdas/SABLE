        ...
        for dim1 in tqdm(dims, desc="Processing dimensions"):
            for dim2 in tqdm(dims, desc=f"Dim {dim1}: Processing dimensions", leave=False):
                for perc_zeros in tqdm(perc_zeros_list, desc=f"Dim {dim2}: Processing % zeros", leave=False):
                    ...
                    vbr_spmv_codegen(fname, dir_name=DENSE_CODEGEN_DIR, vbr_dir=DENSE_VBR_DIR, threads=1, bench=BENCHMARK_FREQ)
                    try:
                    ...
                    f.write("\n")
                    f.flush()
                    os.remove(os.path.join(DENSE_VBR_DIR, fname+".vbr"))
                    os.remove(os.path.join(DENSE_VBR_DIR, fname+".vbrc"))
                    os.remove(os.path.join(SPARSE_VBR_DIR, fname2+".vbr"))
