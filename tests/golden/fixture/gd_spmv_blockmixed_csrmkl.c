...
int main(void) {
    ...
    assert(y != NULL);
    assert(x != NULL);
    double *sable_x0 = (double *)malloc(14 * sizeof(double));
    assert(sable_x0 != NULL);
    ...
    assert(csr_val != NULL);
    double *gd_b = (double *)malloc(14 * sizeof(double));
    assert(gd_b != NULL);
    ...
    read_double_array(matrix_file, gd_b, 14);
    fclose(matrix_file);
    FILE *rhs_file = fopen("<PATH>/x.vector", "r");
    assert(rhs_file != NULL);
    read_dense_input(rhs_file, sable_x0, 14);
    memcpy(x, sable_x0, 14 * sizeof(double));
    fclose(rhs_file);
...
for (long i = 0; i < 14; i++) {
    sable_b_norm += gd_b[i] * gd_b[i];
}
    struct timespec t1, t2;
    double (*dispatch_part_times)[1] = (double (*)[1])calloc(4, 1 * sizeof(double));
    assert(dispatch_part_times != NULL);
    int *sable_iteration_counts = (int *)calloc(1, sizeof(int));
    assert(sable_iteration_counts != NULL);
    double sable_rn;
    for (int iter = 0; iter < 1; iter++) {
        memcpy(x, sable_x0, 14 * sizeof(double));
        sable_rn = DBL_MAX;
        int sable_iteration = 0;
        while (sable_iteration < 100000 && (sable_rn >= sable_b_norm * 0.01 * 0.01)) {
...
for (long i = 0; i < 14; i++) {
    double r = y[i] - gd_b[i];
    sable_rn += r * r;
    x[i] = x[i] - 0.00047938638542665386 * r;
    y[i] = x[i];
}
        clock_gettime(CLOCK_MONOTONIC, &t2);
        ...
        sable_iteration_counts[iter] = sable_iteration;
    }
...
mkl_sparse_destroy(csr_handle);
    ...
    printf("Iterations: ");
    for (int i = 0; i < 1; i++) {
        printf("%d,", sable_iteration_counts[i]);
    }
    printf("\n");
    printf("\n");
    for (int i = 0; i < 14; i++) {
