...
void spmv_sparse_naive(double *restrict y, const double *restrict csr_val, const int *restrict indices, const int *restrict indptr, const double *restrict x, const int rpntr_size) {
...
int main() {
	...
	if (file2 == NULL) { printf("Error opening file2"); return 1; }
		...
		spmv_sparse_naive(y, csr_val, indices, indptr, x, 6);
		clock_gettime(CLOCK_MONOTONIC, &t2);
		sparse_times[i] = (t2.tv_sec - t1.tv_sec) * 1e9 + (t2.tv_nsec - t1.tv_nsec);
		clock_gettime(CLOCK_MONOTONIC, &t1);
		spmv_kernel_mkl(y, x, val, 0, 3, 0, 3, 0);
		clock_gettime(CLOCK_MONOTONIC, &t2);
