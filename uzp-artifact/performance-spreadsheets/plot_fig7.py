...
def gflops_scatterplot( df, version ):
    ...
    plt.ylabel( f"{version} SP-GFLOPS" )
    plt.ylim( [plt.ylim()[0], 17] )
    plt.legend( loc=3, bbox_to_anchor=(0,1) )

    plt.axhline( df.gflops.max(), color="r", linestyle=":", linewidth=3 )
    plt.annotate( "Peak performance: %.02f SP-GFLOPS" % (df.gflops.max()), (10,df.gflops.max()+.1), font = {'weight' : 'bold', 'size' : 12}, color="r"  )#, xytext=None, xycoords='data', textcoords=None, arrowprops=None, annotation_clip=None, **kwargs)

    plt.savefig( f"fig7_{version}.pdf" )
...
if __name__ == "__main__":
    ...
    if len( sys.argv ) < 2:
        print( "Usage: plot_fig7.py [ mkl | uzp | csr | macveth ] <input.csv>" )
        sys.exit( 1 )
    ...
    match version:
        ...
        case "macveth": pass
        case _:
            printf( f"[ERROR] Invalid version: {version}. Must be 'mkl', 'uzp', 'csr', or 'macveth'." )
            sys.exit( 1 )
